...
async def create_door(door: schem_Door.DoorCreate, db: Session = Depends(get_db)):
    ...
    db_door_by_cab = cr_Door.get_doors_cab(db, cabinet_id=door.cabinet_id)
    if not db_door_by_cab:
        raise HTTPException(status_code=404, detail="Данного кабинета не существует")
...
# @rout.get("/doors/type={type}", response_model=list[schem_Door.Door])
# async def read_doors_by_type(type: str, skip: int=0, limit:int=100, db: Session = Depends(get_db)):

#     return cr_Door.read_doors_by_type(db, skip=skip, limit=limit, type=type)

@rout.put("/door/{door_id}")
async def edit_door(door: schem_Door.DoorUpdate, door_id: int, db: Session = Depends(get_db)):
    ...
    db_door_by_num = cr_Door.get_door_by_num_for_upd(db, door_num=door.door_num, door_id=door_id)
    if db_door_by_num:
        raise HTTPException(status_code=400, detail="Запись уже существует")
    
    return cr_Door.update_door(db, door=door, door_id=door_id)

@rout.delete("/door/{door_id}")
