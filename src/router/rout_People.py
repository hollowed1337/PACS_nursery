...
@rout.post("/people", response_model=schem_People.People)
...
@rout.put("/people/{people_id}", response_model=schem_People.People)
async def edit_people(people: schem_People.PeopleUpdate, people_id: int, db:Session = Depends(get_db)):
    ...
    db_peoples_by_role = cr_People.get_peoples_by_role(db, role_id=people.role_id)
    if not db_peoples_by_role:
...
@rout.delete("/people/{people_id}", response_model=schem_People.People)
