...
async def create_child(child: schem_Child.ChildCreate, db: Session = Depends(get_db)):
    ...
    db_child_by_door = cr_Child.get_child_by_door(db, door_id=child.door_id)
    if db_child_by_door:
        raise HTTPException(status_code=401, detail="Дверца уже закреплена за ребенком")
    
    return cr_Child.create_child(db=db, child=child)


@rout.get("/childs/group={group_id}", response_model=list[schem_Child.Child])
async def read_childs_by_group(group_id: int, db: Session = Depends(get_db)):

    db_childs_group = cr_Child.get_childs_group(db, group_id=group_id)
    if not db_childs_group:
        raise HTTPException(status_code=404, detail="Записи не существует")
    
    return cr_Child.get_childs_by_group(db, group_id=group_id)
